...
class Cohere:
    """
    Provider class for interacting with the Cohere API.
    """

    provider_name = "cohere"
    ai_models = [
        {"id": "command-r", "type": "chat.completions", "premium": False},
        {"id": "command-r-plus", "type": "chat.completions", "premium": True}
    ]

    async def stream_response(response: openai.AsyncStream[ChatCompletionChunk], headers: dict[str, str]) -> Stream:
        """Streams the response from the Cohere API."""
    ...
    async def chat_completion(cls, body: Chat) -> typing.Union[Stream, PrettyJSONResponse]:
        ...
        provider = await ProviderManager.get_best_provider_by_model(f"cohere-{body.model}")
        ...
        body.model = f"cohere-{body.model}"
